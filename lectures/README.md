...
01–13회차 페이지는 `packages/web-deck/` 공용 프레임워크를 사용합니다. 01회차는 28장, 02회차는 54장으로 작성되어 있고, 03–13회차는 표지 한 장의 기본 골격으로 남아 있습니다.
...
- 모든 `pre`는 `class="wd-code"`와 `data-wd-code`를 지정합니다. 지원 언어는 `html`, `css`, `javascript`, `jsx`, `json`, `bash`, `text`입니다. 실행 출력은 `samp`로 구분합니다. HTML의 `htmlWhitespaceSensitivity: "ignore"` 설정으로 시작·종료 태그를 읽기 쉽게 정리합니다.
